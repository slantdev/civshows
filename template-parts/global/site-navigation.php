...
<div id="main-nav-wrapper"
  class="main-nav-wrapper relative flex justify-end grow lg:border-b lg:border-white/20 lg:-translate-y-1/2 transition-all duration-1200">
  ...
  <nav class="civ-site-navigation site-navigation hidden lg:flex items-center gap-4 lg:gap-6">
      ...
      <?php foreach ($menu_items as $index => $item): ?>
        ...
        <div class="civ-nav-item group <?php echo $type === 'megamenu' ? 'static' : 'relative'; ?> lg:py-6 xl:py-8">
          <a href="<?php echo esc_url($url); ?>" <?php echo $target; ?>
            class="civ-nav-link flex items-center text-white font-medium uppercase text-sm tracking-wide group-hover:text-shadow-md transition duration-500">
            <?php echo esc_html($title); ?>
            <?php if ($has_children): ?>
              <svg xmlns="http://www.w3.org/2000/svg"
                class="h-4 w-4 ml-1 group-hover:text-shadow-md transition duration-500 transform group-hover:-rotate-180"
          ...
          <?php if ($type === 'megamenu'): ?>
            ...
            ?>
            <div
              class="civ-megamenu-dropdown absolute top-full left-0 w-full bg-white shadow-2xl opacity-0 invisible translate-y-12 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-500 z-50 transform origin-top">
              <div class="grid grid-cols-12 min-h-[450px]">
                ...
                <div class="col-span-3 relative overflow-hidden bg-gray-900">
                  ...
                  <div class="relative z-10 p-10 h-full flex flex-col justify-center text-white">
                    <?php if ($heading): ?>
                      <h2 class="text-3xl font-bold mb-4"><?php echo esc_html($heading); ?></h2><?php endif; ?>
                    <?php if ($desc): ?>
                      <p class="text-sm leading-relaxed text-gray-200"><?php echo esc_html($desc); ?></p><?php endif; ?>
                  </div>
                </div>
                    ...
                    <?php foreach ($mega_items as $sub_index => $sub_item):
                      ...
                      <li>
                        <a href="<?php echo esc_url($sub_link_url); ?>"
                          ...
                          <svg xmlns="http://www.w3.org/2000/svg"
                            class="h-4 w-4 opacity-0 group-hover/link:opacity-100 transition-opacity text-civ-orange-500"
                ...
                <div class="col-span-5 bg-gray-50 p-10 flex">
                  <?php foreach ($mega_items as $sub_index => $sub_item):
                    ...
                    <div id="<?php echo $target_id; ?>"
                      class="megamenu-content civ-megamenu-content w-full <?php echo $hidden_class; ?>">
                      <?php if ($f_img): ?>
                      ...
                      <?php if ($f_link && !empty($f_link['url'])): ?>
                        <a href="<?php echo esc_url($f_link['url']); ?>"
                          class="civ-megamenu-featured-link inline-flex items-center font-medium text-black border-b border-black hover:text-civ-orange-500 hover:border-civ-orange-500 transition-colors">
          ...
          <?php elseif ($type === 'dropdown'): ?>
            ...
            ?>
            <div
              class="civ-simple-dropdown absolute top-full left-0 w-64 bg-white shadow-xl opacity-0 invisible translate-y-4 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0 transition-all duration-300 z-50">
              <ul class="py-2">
                <?php foreach ($drop_items as $d_item):
                  ...
                  <li>
                    <a href="<?php echo esc_url($d_link['url']); ?>"
                      class="civ-dropdown-link block px-6 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-civ-orange-500 transition-colors">
    ...
    <div class="relative flex items-center h-full ml-1" id="nav-search-container">
      <button id="nav-search-toggle"
        class="text-white hover:text-civ-orange-500 transition-colors p-1 relative z-10 cursor-pointer"
        aria-label="Toggle Search">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 font-bold" fill="none" viewBox="0 0 24 24"
          stroke="currentColor" stroke-width="3">
          <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
      </button>

      <form id="nav-search-form" action="<?php echo esc_url(home_url('/')); ?>" method="get"
        class="absolute right-0 top-1/2 -translate-y-1/2 flex items-center bg-white rounded-full shadow-lg overflow-hidden origin-right transition-all duration-300 ease-in-out w-0 opacity-0 invisible z-20"
        style="height: 52px;">
        <input type="text" name="s" placeholder="Search..."
          class="w-full h-full border-none bg-transparent px-4 py-2 text-gray-700 outline-none focus:ring-0 min-w-[200px]"
          required>
        <button type="button" id="nav-search-close"
          class="text-gray-400 hover:text-gray-700 px-3 h-full cursor-pointer transition-colors">
  ...
  if (get_post_type() === 'shows'):
    ...
    if (!empty($buy_ticket_settings)) {
      ...
      if (!empty($btn_data['button_link']['url'])) {
        echo '<div class="flex items-center pr-2 md:pr-4 lg:pl-6 lg:pr-0">';
  ...
  <div class="lg:hidden flex items-center">
    <button id="mobile-menu-toggle" class="civ-mobile-menu-toggle text-white focus:outline-none p-3"
