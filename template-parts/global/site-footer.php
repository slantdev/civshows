...
<footer class="civ-site-footer border-t border-gray-200" style="<?php echo esc_attr($footer_styles); ?>">
  <div class="container mx-auto py-16 px-4 md:px-6 lg:px-8 xl:px-8">
    <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-12 gap-x-4 gap-y-8 lg:gap-8">
      ...
      <div class="col-span-2 md:col-span-3 xl:col-span-5 flex flex-col md:flex-row gap-6 items-start xl:gap-8">
        ...
        <div class="civ-footer-about-content footer-about-content pr-0 xl:pr-6 prose prose-sm xl:max-w-none"
          style="color: inherit;">
          <?php if (!empty($footer_about['about_civ'])): ?>
            <div
              class="[&_h3]:font-semibold [&_h3]:text-[15px] [&_h3]:text-inherit [&_h3]:mb-4 [&_h3]:mt-0 [&_h3]:leading-tight [&_p]:text-[13px] [&_p]:leading-relaxed">
              <?php echo $footer_about['about_civ']; ?>
            </div>
          <?php endif; ?>
          <?php if (!empty($footer_social['social_media'])): ?>
            <div class="flex items-center gap-6 not-prose mt-4">
              <?php foreach ($footer_social['social_media'] as $social):
                $platform = $social['social_media'] ?? '';
                $link = $social['link'] ?? [];
                if ($link && $platform):
                  ?>
                  <a href="<?php echo esc_url($link['url']); ?>" target="<?php echo esc_attr($link['target'] ?: '_blank'); ?>"
      ...
      <div class="xl:col-span-2 space-y-6 xl:space-y-8">
        <!-- Navigation -->
        <div class="civ-footer-navigation">
          <?php if (!empty($footer_nav['heading'])): ?>
            <h3 class="font-bold text-base md:text-lg xl:text-lg mb-4" style="color: inherit; opacity: 0.8;">
              <?php echo esc_html($footer_nav['heading']); ?>
            </h3>
          <?php endif; ?>
        ...
        <!-- Contacts -->
        <div class="civ-footer-contacts">
          <?php if (!empty($footer_contacts['heading'])): ?>
            <h3 class="font-bold text-base md:text-lg xl:text-lg mb-4 lg:mb-6" style="color: inherit; opacity: 0.8;">
              <?php echo esc_html($footer_contacts['heading']); ?>
            </h3>
          <?php endif; ?>
          ...
          <div class="space-y-4 text-sm">
            ...
            <?php if (!empty($footer_contacts['phone'])): ?>
              <p>
                <a href="tel:<?php echo esc_attr(str_replace(' ', '', $footer_contacts['phone'])); ?>"
                  class="civ-footer-phone-link hover:opacity-70 transition-colors font-medium">
      ...
      <!-- Subscribe -->
      <div class="col-span-2 md:col-span-2 xl:col-span-5">
        <?php if (!empty($footer_subscribe['enable_subscribe'])): ?>
          <h3 class="font-bold text-base md:text-lg xl:text-lg mb-4" style="color: inherit; opacity: 0.8;">
            <?php echo esc_html($footer_subscribe['subscribe_heading']); ?>
          </h3>

          <?php if ($footer_subscribe['snippets']): ?>
            <?php echo $footer_subscribe['snippets']; ?>
          <?php endif; ?>
  ...
  ?>
  <div class="civ-footer-copyright py-4 w-full text-white"
    style="background-color: <?php echo esc_attr($copyright_bg_color); ?>;">
    <div
      class="container mx-auto px-4 xl:px-8 text-sm font-medium flex flex-col md:flex-row justify-between items-center gap-4">
      <div>
