<?php

/**
 * Component Visitor Information
 */

?>

<section class="w-full bg-white flex flex-col lg:flex-row">

  <div class="w-full lg:w-5/12 relative min-h-[400px] lg:min-h-auto">
    <img src="https://placehold.co/800x1200/e2e8f0/475569?text=Map+Location+View" alt="Map of Sandown Racecourse" class="absolute inset-0 w-full h-full object-cover">
  </div>

  <div class="w-full lg:w-7/12 px-6 py-12 md:px-12 md:py-16 lg:px-20 lg:py-24">

<?php

/**
 * Component Visitor Information
 */

?>

<section class="w-full bg-white flex flex-col lg:flex-row">

  <div class="w-full lg:w-5/12 relative min-h-[400px] lg:min-h-auto">
    <img src="https://placehold.co/800x1200/e2e8f0/475569?text=Map+Location+View" alt="Map of Sandown Racecourse" class="absolute inset-0 w-full h-full object-cover">
  </div>

  <div class="w-full lg:w-7/12 px-6 py-12 md:px-12 md:py-16 lg:px-20 lg:py-24">

    <h2 class="text-3xl md:text-4xl font-extrabold text-black mb-6">Visitor Information</h2>
    <p class="text-base md:text-lg xl:text-lg text-gray-800 mb-10 font-medium">
      Visit the BIGGEST & BEST Caravan & Camping event in the east!
    </p>

    <div class="space-y-6 mb-12">
      <div>
        <h4 class="font-bold uppercase text-black mb-1">When</h4>
        <p class="text-gray-700">Thursday 18 – Sunday 21 September 2025</p>
      </div>

      <div>
        <h4 class="font-bold uppercase text-black mb-1">Where</h4>
        <p class="text-gray-700 mb-2">
          Sandown Racecourse,<br>
          591-659 Princes Hwy, Springvale VIC 3171
        </p>
        <a href="#" class="inline-block text-civ-orange-500 font-bold hover:text-civ-orange-600 transition-colors uppercase">
          &gt; Get Directions
        </a>
      </div>
    </div>

    <div class="border-t border-gray-200 my-10"></div>

    <div class="mb-12">
      <h3 class="font-extrabold text-base md:text-lg xl:text-lg uppercase text-black mb-6">Event Trading Hours</h3>

      <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Thursday 18 September 2025</span>
            <span class="text-gray-600">9:00am to 4:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Friday 19 September 2025</span>
            <span class="text-gray-600">9:00am to 4:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Saturday 20 September 2025</span>
            <span class="text-gray-600">9:00am to 5:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Sunday 21 September 2025</span>
            <span class="text-gray-600">9:00am to 4:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

      </div>
    </div>

    <div class="border-t border-gray-200 my-10"></div>

    <div>
      <h3 class="font-extrabold text-lg uppercase text-black mb-6">Getting There</h3>
      <p class="text-gray-700 mb-8">
        Getting to the Melbourne Caravan & Camping Leisurefest is easy by car or public transport.
      </p>

      <ul class="space-y-8">
        <li class="flex gap-6">
          <div class="shrink-0 mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black" viewBox="0 0 20 20" fill="currentColor">
              <path d="M8 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM15 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z" />
              <path d="M3 4a1 1 0 00-1 1v10a1 1 0 001 1h1.05a2.5 2.5 0 014.9 0H10a1 1 0 001-1V5a1 1 0 00-1-1H3zM14 7a1 1 0 00-1 1v6.05A2.5 2.5 0 0115.95 16H17a1 1 0 001-1v-5a1 1 0 00-.293-.707l-2-2A1 1 0 0015 7h-1z" />
            </svg>
          </div>
          <div>
            <h4 class="font-bold uppercase text-black mb-1">Car Parking Information</h4>
            <p class="text-gray-700 leading-relaxed">
              There is free parking available at the Sandown Racecourse for all days of the show! We will have car parking staff to help and direct you to available parking locations throughout the show period.
            </p>
          </div>
        </li>

        <li class="flex gap-6">
          <div class="shrink-0 mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2a2 2 0 100 4 2 2 0 000-4z" />
              <path fill-rule="evenodd" d="M13.293 6.293a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414l-2.586-2.586V13.5a1.5 1.5 0 01-3 0V9.914l-1.293 1.293a1 1 0 01-1.414-1.414l4-4a1 1 0 010-1.086zM5 19a1 1 0 011-1h12a1 1 0 110 2H6a1 1 0 01-1-1z" clip-rule="evenodd" />
              <path d="M10 16a2 2 0 100 4 2 2 0 000-4zM16 16a2 2 0 100 4 2 2 0 000-4z" />
              <path d="M6 10a1 1 0 011-1h.22a2 2 0 011.993 1.838l.4 4.5a1 1 0 01-.996 1.09L7 16.5a1 1 0 01-1-1v-5.5z" />
            </svg>
          </div>
          <div>
            <h4 class="font-bold uppercase text-black mb-1">Disabled Parking & Access</h4>
            <p class="text-gray-700 leading-relaxed">
              Dedicated disabled parking spaces are available at the Sandown Racecourse. The Sandown Racecourse is a fully accessible venue with disabled access to all pavilions and buildings.
            </p>
          </div>
        </li>

        <li class="flex gap-6">
          <div class="shrink-0 mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black" viewBox="0 0 20 20" fill="currentColor">
              <path d="M4 3a2 2 0 100 4h12a2 2 0 100-4H4z" />
              <path fill-rule="evenodd" d="M3 8h14v7a2 2 0 01-2 2H5a2 2 0 01-2-2V8zm5 3a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1z" clip-rule="evenodd" />
            </svg>
          </div>
          <div>
            <h4 class="font-bold uppercase text-black mb-1">Public Transport Information</h4>
            <p class="text-gray-700 leading-relaxed">
              Trains will be available on all days of the show! The closest station is Sandown Park. To plan your journey on public transport to the Sandown Racecourse, use Public Transport Victoria's Journey Planner.
            </p>
          </div>
        </li>
      </ul>

    </div>

  </div>
</section>

    <div class="space-y-6 mb-12">
      <div>
        <h4 class="font-bold uppercase text-black mb-1">When</h4>
        <p class="text-gray-700">Thursday 18 – Sunday 21 September 2025</p>
      </div>

      <div>
        <h4 class="font-bold uppercase text-black mb-1">Where</h4>
        <p class="text-gray-700 mb-2">
          Sandown Racecourse,<br>
          591-659 Princes Hwy, Springvale VIC 3171
        </p>
        <a href="#" class="inline-block text-civ-orange-500 font-bold hover:text-civ-orange-600 transition-colors uppercase">
          &gt; Get Directions
        </a>
      </div>
    </div>

    <div class="border-t border-gray-200 my-10"></div>

<?php

/**
 * Component Visitor Information
 */

?>

<section class="w-full bg-white flex flex-col lg:flex-row">

  <div class="w-full lg:w-5/12 relative min-h-[400px] lg:min-h-auto">
    <img src="https://placehold.co/800x1200/e2e8f0/475569?text=Map+Location+View" alt="Map of Sandown Racecourse" class="absolute inset-0 w-full h-full object-cover">
  </div>

  <div class="w-full lg:w-7/12 px-6 py-12 md:px-12 md:py-16 lg:px-20 lg:py-24">

    <h2 class="text-3xl md:text-4xl font-extrabold text-black mb-6">Visitor Information</h2>
    <p class="text-base md:text-lg xl:text-lg text-gray-800 mb-10 font-medium">
      Visit the BIGGEST & BEST Caravan & Camping event in the east!
    </p>

    <div class="space-y-6 mb-12">
      <div>
        <h4 class="font-bold uppercase text-black mb-1">When</h4>
        <p class="text-gray-700">Thursday 18 – Sunday 21 September 2025</p>
      </div>

      <div>
        <h4 class="font-bold uppercase text-black mb-1">Where</h4>
        <p class="text-gray-700 mb-2">
          Sandown Racecourse,<br>
          591-659 Princes Hwy, Springvale VIC 3171
        </p>
        <a href="#" class="inline-block text-civ-orange-500 font-bold hover:text-civ-orange-600 transition-colors uppercase">
          &gt; Get Directions
        </a>
      </div>
    </div>

    <div class="border-t border-gray-200 my-10"></div>

    <div class="mb-12">
      <h3 class="font-extrabold text-base md:text-lg xl:text-lg uppercase text-black mb-6">Event Trading Hours</h3>

      <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Thursday 18 September 2025</span>
            <span class="text-gray-600">9:00am to 4:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Friday 19 September 2025</span>
            <span class="text-gray-600">9:00am to 4:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Saturday 20 September 2025</span>
            <span class="text-gray-600">9:00am to 5:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Sunday 21 September 2025</span>
            <span class="text-gray-600">9:00am to 4:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

      </div>
    </div>

    <div class="border-t border-gray-200 my-10"></div>

    <div>
      <h3 class="font-extrabold text-base md:text-lg xl:text-lg uppercase text-black mb-6">Getting There</h3>
      <p class="text-gray-700 mb-8">
        Getting to the Melbourne Caravan & Camping Leisurefest is easy by car or public transport.
      </p>

      <ul class="space-y-8">
        <li class="flex gap-6">
          <div class="shrink-0 mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black" viewBox="0 0 20 20" fill="currentColor">
              <path d="M8 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM15 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z" />
              <path d="M3 4a1 1 0 00-1 1v10a1 1 0 001 1h1.05a2.5 2.5 0 014.9 0H10a1 1 0 001-1V5a1 1 0 00-1-1H3zM14 7a1 1 0 00-1 1v6.05A2.5 2.5 0 0115.95 16H17a1 1 0 001-1v-5a1 1 0 00-.293-.707l-2-2A1 1 0 0015 7h-1z" />
            </svg>
          </div>
          <div>
            <h4 class="font-bold uppercase text-black mb-1">Car Parking Information</h4>
            <p class="text-gray-700 leading-relaxed">
              There is free parking available at the Sandown Racecourse for all days of the show! We will have car parking staff to help and direct you to available parking locations throughout the show period.
            </p>
          </div>
        </li>

        <li class="flex gap-6">
          <div class="shrink-0 mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2a2 2 0 100 4 2 2 0 000-4z" />
              <path fill-rule="evenodd" d="M13.293 6.293a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414l-2.586-2.586V13.5a1.5 1.5 0 01-3 0V9.914l-1.293 1.293a1 1 0 01-1.414-1.414l4-4a1 1 0 010-1.086zM5 19a1 1 0 011-1h12a1 1 0 110 2H6a1 1 0 01-1-1z" clip-rule="evenodd" />
              <path d="M10 16a2 2 0 100 4 2 2 0 000-4zM16 16a2 2 0 100 4 2 2 0 000-4z" />
              <path d="M6 10a1 1 0 011-1h.22a2 2 0 011.993 1.838l.4 4.5a1 1 0 01-.996 1.09L7 16.5a1 1 0 01-1-1v-5.5z" />
            </svg>
          </div>
          <div>
            <h4 class="font-bold uppercase text-black mb-1">Disabled Parking & Access</h4>
            <p class="text-gray-700 leading-relaxed">
              Dedicated disabled parking spaces are available at the Sandown Racecourse. The Sandown Racecourse is a fully accessible venue with disabled access to all pavilions and buildings.
            </p>
          </div>
        </li>

        <li class="flex gap-6">
          <div class="shrink-0 mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black" viewBox="0 0 20 20" fill="currentColor">
              <path d="M4 3a2 2 0 100 4h12a2 2 0 100-4H4z" />
              <path fill-rule="evenodd" d="M3 8h14v7a2 2 0 01-2 2H5a2 2 0 01-2-2V8zm5 3a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1z" clip-rule="evenodd" />
            </svg>
          </div>
          <div>
            <h4 class="font-bold uppercase text-black mb-1">Public Transport Information</h4>
            <p class="text-gray-700 leading-relaxed">
              Trains will be available on all days of the show! The closest station is Sandown Park. To plan your journey on public transport to the Sandown Racecourse, use Public Transport Victoria's Journey Planner.
            </p>
          </div>
        </li>
      </ul>

    </div>

  </div>
</section>
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Thursday 18 September 2025</span>
            <span class="text-gray-600">9:00am to 4:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Friday 19 September 2025</span>
            <span class="text-gray-600">9:00am to 4:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Saturday 20 September 2025</span>
            <span class="text-gray-600">9:00am to 5:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div class="flex items-center gap-8 w-full sm:w-auto">
            <span class="text-gray-800 font-medium w-60">Sunday 21 September 2025</span>
            <span class="text-gray-600">9:00am to 4:00pm</span>
          </div>
          <button class="bg-civ-orange-500 hover:bg-civ-orange-600 text-sm text-white font-bold uppercase py-2 px-4 rounded-sm flex items-center gap-2 transition-colors shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            Add iCal or Outlook
          </button>
        </div>

      </div>
    </div>

    <div class="border-t border-gray-200 my-10"></div>

    <div>
      <h3 class="font-extrabold text-lg uppercase text-black mb-6">Getting There</h3>
      <p class="text-gray-700 mb-8">
        Getting to the Melbourne Caravan & Camping Leisurefest is easy by car or public transport.
      </p>

      <ul class="space-y-8">
        <li class="flex gap-6">
          <div class="shrink-0 mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black" viewBox="0 0 20 20" fill="currentColor">
              <path d="M8 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM15 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z" />
              <path d="M3 4a1 1 0 00-1 1v10a1 1 0 001 1h1.05a2.5 2.5 0 014.9 0H10a1 1 0 001-1V5a1 1 0 00-1-1H3zM14 7a1 1 0 00-1 1v6.05A2.5 2.5 0 0115.95 16H17a1 1 0 001-1v-5a1 1 0 00-.293-.707l-2-2A1 1 0 0015 7h-1z" />
            </svg>
          </div>
          <div>
            <h4 class="font-bold uppercase text-black mb-1">Car Parking Information</h4>
            <p class="text-gray-700 leading-relaxed">
              There is free parking available at the Sandown Racecourse for all days of the show! We will have car parking staff to help and direct you to available parking locations throughout the show period.
            </p>
          </div>
        </li>

        <li class="flex gap-6">
          <div class="shrink-0 mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2a2 2 0 100 4 2 2 0 000-4z" />
              <path fill-rule="evenodd" d="M13.293 6.293a1 1 0 011.414 0l4 4a1 1 0 01-1.414 1.414l-2.586-2.586V13.5a1.5 1.5 0 01-3 0V9.914l-1.293 1.293a1 1 0 01-1.414-1.414l4-4a1 1 0 010-1.086zM5 19a1 1 0 011-1h12a1 1 0 110 2H6a1 1 0 01-1-1z" clip-rule="evenodd" />
              <path d="M10 16a2 2 0 100 4 2 2 0 000-4zM16 16a2 2 0 100 4 2 2 0 000-4z" />
              <path d="M6 10a1 1 0 011-1h.22a2 2 0 011.993 1.838l.4 4.5a1 1 0 01-.996 1.09L7 16.5a1 1 0 01-1-1v-5.5z" />
            </svg>
          </div>
          <div>
            <h4 class="font-bold uppercase text-black mb-1">Disabled Parking & Access</h4>
            <p class="text-gray-700 leading-relaxed">
              Dedicated disabled parking spaces are available at the Sandown Racecourse. The Sandown Racecourse is a fully accessible venue with disabled access to all pavilions and buildings.
            </p>
          </div>
        </li>

        <li class="flex gap-6">
          <div class="shrink-0 mt-1">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-black" viewBox="0 0 20 20" fill="currentColor">
              <path d="M4 3a2 2 0 100 4h12a2 2 0 100-4H4z" />
              <path fill-rule="evenodd" d="M3 8h14v7a2 2 0 01-2 2H5a2 2 0 01-2-2V8zm5 3a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1z" clip-rule="evenodd" />
            </svg>
          </div>
          <div>
            <h4 class="font-bold uppercase text-black mb-1">Public Transport Information</h4>
            <p class="text-gray-700 leading-relaxed">
              Trains will be available on all days of the show! The closest station is Sandown Park. To plan your journey on public transport to the Sandown Racecourse, use Public Transport Victoria's Journey Planner.
            </p>
          </div>
        </li>
      </ul>

    </div>

  </div>
</section>