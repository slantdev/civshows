...
- **ACFE Ultra Mode Compatibility**: Updated `WP_Query` meta queries in `template-parts/sections/exhibitors_shows.php` and `inc/ajax-handlers.php` to search the `acf` meta key instead of specific field names, fixing exhibitor filtering when ACFE performance mode is set to 'ultra'.
...
- **Interactive Select Component:** Integrated automated viewport boundary detection within `src/main.js` to dynamically swap between "dropdown" and "drop-up" positioning classes based on available screen space, ensuring the menu natively sits on top of and fully covers the trigger when required without getting cut off below the viewport fold. Adjusted responsive margin and padding (`pt/pb/py`) constraints.
- **Home Hero Slider Section:** Refined container heights (`h-[700px]`, `lg:h-svh`), responsive subtitle line-clamping logic, slider typography sizes, and container padding directives across desktop (`lg`, `xl`, `2xl`) breakpoints for enhanced scaling alignment.
